...
export const oliveJournal = createTheme({
  ...
  components: {
    cover: (data) => {
      ...
      const footTags = tags.length
        ? `<section style="display:flex;gap:6px;">${tags.map((t) => `<span style="color:${v.secondary};font-size:10px;font-weight:700;letter-spacing:0.5px;">${wrapLeaf('#' + escapeHtml(t))}</span>`).join('')}</section>`
        : ''
      const foot = footLeft || footTags
        ? `<section style="padding:10px 26px;display:flex;align-items:center;justify-content:space-between;border-top:3px solid ${v.secondary};background:${v.lightBg};">${footLeft}${footTags}</section>`
        : ''
  ...
  <section style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
    <section style="width:28px;height:28px;flex-shrink:0;background:${v.primary};border-radius:4px;display:flex;align-items:center;justify-content:center;"><span style="font-size:13px;font-weight:800;color:#fff;">${wrapLeaf(escapeHtml(data.num))}</span></section>
  ...
  <section style="width:100%;height:1px;background:${v.borderColor};margin-top:12px;"><span leaf=""><br></span></section>
    ...
    signature: (data) => {
      ...
      return `<section style="margin:40px 20px 24px;padding:24px 22px;background:${v.lightBg};border-radius:6px;border-top:3px solid ${v.primary};">
